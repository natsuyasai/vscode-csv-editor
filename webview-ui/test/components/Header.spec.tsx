...
vi.mock("@vscode-elements/react-elements", () => ({
  ...
  VscodeButton: ({
    ...
    return (
      <button {...buttonProps} {...(secondary ? { secondary: "true" } : {})}>
        {children}
      </button>
    );
  },
...
describe("Header", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // セレクター関数に対応したモック設定
    const mockStore = useAlignmentModeStore as unknown as {
      mockImplementation: (
        fn: (selector?: (state: typeof mockUseAlignmentModeStore) => unknown) => unknown
      ) => void;
    };
